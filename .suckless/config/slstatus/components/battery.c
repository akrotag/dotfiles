...
*/
		...
		return (i == LEN(map)) ? "3" : map[i].symbol;
	}


  const char *
  get_battery_symbol(const char *state, const int perc)
  {
    //{"󰢟", "󱊤", "󱊥", "󱊦"}}
    //{{"󰂎", "󱊡", "󱊢", "󱊣"}};
    char *symbols[4];  
  
    if (strcmp(state, "o")==0){ return "󱊣";}
    if(strcmp(state, "+")==0){
      symbols[0] = "󰢟";
      symbols[1] = "󱊤";
      symbols[2] = "󱊥";
    ...
    const char *state = battery_state(bat);
		//const char *bat_full = "󱊣";

    /*if(strcmp(state, "+")){
      //bat_icons = icons_struct;
      return bprintf("%s apagnan %s , %d", bat_full, state, perc);
    }
    else if (strcmp(state, "3")){
      return NULL;
    } 
    else if (strcmp(state, "2")){
      //return bprintf("%s %d", bat_full, perc);
    }
    else{return NULL;}
    */
		return bprintf("%s %d", get_battery_symbol(state, perc), perc);
	}
